var Ranger = function(range, options) {
  ...
  // Don't be a bitch if provided elements are jQuery
  
  if (Ranger.utils.isJQuery(range.startContainer)) range.startContainer = range.startContainer[0];
  if (Ranger.utils.isJQuery(range.endContainer)) range.endContainer = range.endContainer[0];
  if (Ranger.utils.isJQuery(range.commonAncestorContainer)) range.commonAncestorContainer = range.commonAncestorContainer[0];
  if (Ranger.utils.isJQuery(options.context)) options.context = options.context[0];
  ...
  // Find start, end and offsets
  
  if (typeof range.startContainer === 'string') {
    this._range.startContainer = Ranger.utils.xpath.getElement(range.startContainer, this._range.context);
  } else {
    this._range.startContainer = range.startContainer;
  }
  
  if (typeof range.endContainer === 'string') {
    this._range.endContainer = Ranger.utils.xpath.getElement(range.endContainer, this._range.context);
  } else {
...
Ranger.prototype.toJSON = function() {
  ...
  var serialize = function(elem) {
    ...
    var skipElements = [].slice.call(document.querySelectorAll(skipSelector));
    ...
    var xpath = Ranger.utils.xpath.getXPath(parent, this._range.context, skipElements);
...
Ranger.utils = {
  ...
  isJQuery: function(elem) {
    return elem && elem[0] && typeof elem.selector == 'string';
  },
  ...
  xpath: {
    ...
    getXPath: function(elem, contextNode, ignoreNodes) {
      ...
      if (contextNode !== document) path = "." + path;
