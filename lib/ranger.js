var Ranger = function(range, options) {
  ...
  if (typeof this._range.startContainer === 'string') {
    this._range.startContainer = Ranger.utils.xpath.getElement(this._range.startContainer, this._range.context);
  }
  
  if (typeof this._range.endContainer === 'string') {
    this._range.endContainer = Ranger.utils.xpath.getElement(this._range.endContainer, this._range.context);
  }
...
Ranger.utils = {
  ...
  notEmpty: function(node) {
    return !Ranger.utils.whitespaceRegexp.test(node.nodeValue);
  },
  ...
  collectTextNodes: function(elem, start, end, occured) {
    ...
    return children.reduce(function(nodes, child) {
      ...
      if (occured.start && child.nodeType === 3 && Ranger.utils.notEmpty(child)) {
        nodes.push(child);
