var Ranger = function(range) {
  ...
  if (!(range && range.startContainer && range.endContainer && isInteger(range.startOffset) && isInteger(range.endOffset))) {
...
Ranger.prototype.textNodes = function() {
  ...
  if (this._textNodes) return this._textNodes; // cache computation
  ...
  if (start.nodeType === 1 && startOffset > 0) {
    start = Ranger._findTextNode(start, startOffset);
    this._range.startContainerTextNode = start;
  }
  
  if (end.nodeType === 1 && endOffset > 0) {
    end = Ranger._findTextNode(end, endOffset);
    this._range.endContainerTextNode = end;
  }
  ...
  if (start == end) {
    if ((endOffset - startOffset) < newRange.start.nodeValue.length) {
...
Ranger.prototype.paint = function(wrapperClass) {
  ...
  return this.textNodes().map(function(node) {
...
Ranger.prototype.toString = function() {

  return this.textNodes().reduce(function(s, current) {
...
Ranger.prototype.toJSON = function(context, ignoreClassName) {
  ...
  var serialize = function(elem) {
    ...
    // this._range.endContainerTextNode
    
    if (elem.nodeType === 3) {
      parentElement = elem.parentNode;
      
      while (parentElement.className === ignoreClassName) {
        parentElement = parentElement.parentNode;
      };
      
      var textNodes = Ranger.collectTextNodes(parentElement).slice(0, textNodes.indexOf(elem));
    ...
    var xpath = Ranger.xpath.getXPath(parentElement, context);
...
Ranger.collectTextNodes = function(elem, start, end, occured) {
  ...
  return children.reduce(function(nodes, child) {
    ...
    if (occured.end) return nodes;
    
    if (child == start) occured.start = true;
    if (child == end) occured.end = true;

    if (occured.start && child.nodeType === 3) { //  && !white.test(child.nodeValue) // white = /^[\s\n]*$/;
      nodes.push(child);
...
Ranger._findTextNode = function(elem, offset) {
  var count = 0, i = 0, node,
    nodes = Ranger.collectTextNodes(elem);
...
Ranger.xpath = {
  ...
  getElement: function (xpath, contextNode) {
    ...
    return document.evaluate(xpath, contextNode, null, XPathResult.FIRST_ORDERED_NODE_TYPE).singleNodeValue;
  },
  
  getElements: function (xpath, contextNode) {
    contextNode = contextNode || document;
    var iterator = document.evaluate(xpath, contextNode, null, XPathResult.ANY_TYPE),
