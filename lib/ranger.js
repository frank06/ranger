var Ranger = function(range, options) {
  ...
  // if start/end are jQuery, don't be a bitch
  
  if (Ranger.utils.isJQuery(range.startContainer)) this._range.startContainer = range.startContainer[0];
  if (Ranger.utils.isJQuery(range.endContainer)) this._range.endContainer = range.endContainer[0];
  if (Ranger.utils.isJQuery(range.commonAncestorContainer)) this._range.commonAncestorContainer = range.commonAncestorContainer[0];
  if (Ranger.utils.isJQuery(options.context)) this._range.context = options.context[0];
