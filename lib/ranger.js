var Ranger = function(range, options) {
  ...
  this._range.document = options.document || document;
  ...
  if (Ranger.utils.isTextNode(start)) {
    var offset = Ranger.utils.safeLength(startOffset, start);
    newRange.start = start.splitText(offset);
  ...
  if (start == end) {
    if (Ranger.utils.isTextNode(newRange.start)) {
      var offsetDiff = Ranger.utils.safeLength(endOffset - startOffset, end);
      newRange.start.splitText(offsetDiff);
      this.ie9Workaround(newRange.start);
    }
    newRange.end = newRange.start;
  } else {
    if (Ranger.utils.isTextNode(end)) {
      var offset = Ranger.utils.safeLength(endOffset, end);
...
Ranger.utils = {
  ...
  safeLength: function(offset, node) {
    return (offset > node.length) ? node.length : offset;
  },
